...
/**
 * Class that imports an XML-file and transforms it to a Molecule. It supports
 * the xml-form explained in Easymol wiki and has support for both recursive and
 * linear atom structure.
 * 
 * @author Aleksi Suomalainen
 */
public class XMLtoMolecule {
    ...
    /**
     * Scan through org.w3c.dom.Element named atom. This elements supports both
     * recursive and linear structure.
     */
    void visitElement_atom(org.w3c.dom.Element element) {
        ...
        if (element.getParentNode() != null) {
            ...
            if (element.getParentNode().getNodeName().equals("atom")) {
                frombonds.add(element.getParentNode().getAttributes()
                        .getNamedItem("bondid").getNodeValue());
                tobonds.add(element.getAttributes().getNamedItem("bondid")
                        .getNodeValue());
            }
        }
        org.w3c.dom.NamedNodeMap attrs = element.getAttributes();
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
            ...
            if (attr.getName().equals("name")) { // <atom name="???">
                ...
                if ((attr.getValue()).equalsIgnoreCase("O")) {
                    a = new Atom(Atom.O);
                    a.setUniqueId(element.getAttributes().getNamedItem("bondid")
                            .getNodeValue());
                    mol.addCompound(a);
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            switch (node.getNodeType()) {
            /*
             * for future use. case org.w3c.dom.Node.CDATA_SECTION_NODE: //
             * ((org.w3c.dom.CDATASection)node).getData(); break; case
             * org.w3c.dom.Node.PROCESSING_INSTRUCTION_NODE: //
             * ((org.w3c.dom.ProcessingInstruction)node).getTarget(); //
             * ((org.w3c.dom.ProcessingInstruction)node).getData(); break; case
             * org.w3c.dom.Node.TEXT_NODE: //
             * ((org.w3c.dom.Text)node).getData(); break;
             */
            case org.w3c.dom.Node.ELEMENT_NODE:
                org.w3c.dom.Element nodeElement = (org.w3c.dom.Element) node;
                if (nodeElement.getTagName().equals("atom")) {
    ...
    void visitElement_bond(org.w3c.dom.Element element) { // <bond>
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
            ...
            if (attr.getName().equals("valency")) {
                valencies.put(frombonds.lastElement() + "->"
                        + tobonds.lastElement(), attr.getValue());
    ...
    public XMLtoMolecule(File f) {
        try {
            ...
            DocumentBuilderFactory builderFactory = javax.xml.parsers.DocumentBuilderFactory
                    .newInstance();
        ...
        while (fromCompounds.hasMoreElements()) {
            while (toCompounds.hasMoreElements()) {
                ...
                try {
                    ...
                    mol.addBond(new ValencyBond((Atom) mol
                            .getCompoundByUniqueId(first), (Atom) mol
                            .getCompoundByUniqueId(second), (valencies.get(first
                            + "->" + second) != null) ? Integer
                            .parseInt((String) valencies.get(first + "->"
                                    + second)) : 1));
