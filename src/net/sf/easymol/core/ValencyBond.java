...
public class ValencyBond extends AbstractChemicalBond {
    ...
    /**
     * Constructs a valency bond out of two atoms and specified bonds type.
     * 
     * @param First
     *            atom.
     * @param Second
    ...
    public ValencyBond(Atom first, Atom second, int type)
        ...
        if (first.getSatisfiedValency() == first.getValency())
            throw new EasyMolException(101, "Atom " + first.getSymbol(), "Atom "
                    + first.getSymbol() + " valency is already satisfied");
        if (second.getSatisfiedValency() == second.getValency())
            throw new EasyMolException(101, "Atom " + second.getSymbol(), "Atom "
                    + second.getSymbol() + " valency is already satisfied");           
